...
- **兵马未动，粮草先行（开工第一宪法，先侦察后动手）**：
  **工具没准备好，绝不先开始干活。** 开工挖掘之前，必须先完成四步准备，
  ...
  2. **调研（硬性前置：未经实际搜索不得输出方案，禁止"记忆库偷懒"）**：
     **任何工具选择、测试方案、挖掘思路，必须先执行真实的网络搜索**
     （GitHub 搜索 API / Web 搜索），**再基于搜索结果作答**——不是凭自己
     记忆里的工具库直接输出：
     - **记忆库不是依据**：我脑子里已有的工具（fuzz/postmcp/playwright/
       cargo-fuzz…）只算"候选种子"，**必须先用它们去搜索验证是否仍是当前
       最优**，再确认使用。换项目/换技术栈时，记忆库覆盖不到的领域
       **必须以搜索为准**——搜索是"我的记忆之外的世界"，不搜 = 我的世界
       只有记忆那么大，通用性就没了。
     - **跳过搜索直接用记忆作答 = 偷懒逃逸**：直接靠记忆输出工具清单、
       不先搜索验证，是违反本宪法的行为（见反模式「记忆库偷懒」）。搜索
       不是可选项，是**不可跳过的固定动作**。
     - **做法**：用 GitHub 搜索 / Web 搜索查该项目技术栈生态里**活跃维护、
       star 高、专门针对该场景**的工具（例：TUI → 搜 "tui testing tool" 找到
       agent-tty/pexpect；Rust 桌面 → 搜 "egui testing" / "tauri e2e" 找到
       egui-driver/tauri-webdriver；解析器 → 搜该格式的 fuzz 器）。
     - **有成熟开源工具就不自研**（反模式：重复造轮子）；只有现成工具都不
       覆盖的场景才自研/自造。
     - 调研产出 = 候选工具清单 + 推荐理由 + **搜索来源引用**，写进本轮的
       准备记录。
...
- **工欲善其事，必先利其器（工具即杠杆，先排查后动手）**：别拿原始手段硬凿。
  ...
  **记忆库是候选种子，搜索才是依据**：我自己已知的工具（fuzz/postmcp/
  playwright/cargo-fuzz…）只是"候选"，**必须用真实搜索（GitHub API/Web）
  验证它是否仍是当前该场景的最优解**，尤其换技术栈时——我记忆覆盖不到的
  领域，搜索就是唯一的真相来源。**搜索是行为不是记忆**：通用性不来自"我
  记住了多少工具"，而来自"每次方案前必先搜索"这个不可跳过的动作。记忆库
  偷懒（跳过搜索直接输出）= 通用性崩塌（反模式）。
...
0. **模式确定**（宪法）：
 ...
 0.5. **准备（兵马未动，粮草先行）**：**全新循环必做，工具没备好绝不开工**——
    ...
    有什么功能、技术栈、入口、测试体系；② 调研：**必须实际搜索**（GitHub/Web）
    ——按项目材质搜网上最新方案，**未经搜索不得输出工具清单**（禁止记忆库
    偷懒）；③ 定策：**先定测试类型（黑盒/白盒/自动化），再按项目类型选具体
    工具**——黑盒：Web→
...
## 工作流程（每轮内的完整动作）
 ...
 0. **模式确定**（宪法）：全新启动询问用户「是否自动修复 bug？」（auto/log-only，
    ...
    有什么功能、技术栈、入口、测试体系；② 调研：**必须实际搜索**（GitHub/Web）
    网上最新方案，**未经搜索不得输出工具清单**（禁止记忆库偷懒）；③ 定策：**先定测试类型（黑盒/白盒/自动化）再按项目
...
- ❌ **记忆库偷懒（违兵马未动·调研宪法，最高频逃逸）**：遇到新项目/新技术栈，
  **跳过真实搜索、直接凭自己记忆里的工具库输出方案**——这是最阴险的偷懒：
  输出看起来专业，实际是"我的记忆世界"的产物，不是"真实世界"的产物。
  后果：换项目类型就露馅（记忆库覆盖不到 → 方案残缺），只能事后打补丁
  更新版本。**任何工具选择/测试方案，必须先执行真实搜索（GitHub API/Web）
  再作答**；记忆库只算候选种子，必须用搜索验证它是否仍是当前最优。搜索是
  不可跳过的固定动作，不是可选项——跳过一次 = 通用性崩塌一次。

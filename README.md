...
- **Reset 需交互确认**：死亡后不自动重置计数——仅在用户交互场景明确回复确认后
  才调用 `verify_life.py reset`。
...
> 并提交，然后提示你重置计数数据（agent 自己无权运行 reset）：
...
> Reset 不自动执行——需你在交互中明确回复确认后，才调用
> `verify_life.py reset` 重置计数数据，为下一个会话做准备。

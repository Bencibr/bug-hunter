...
- **测试分层（效率）+ 深度递进 + 覆盖引导（挖掘）**：冒烟（秒级快筛）→ 聚焦
 ...
 - **兵马未动，粮草先行（开工第一宪法）**：工具没准备好，绝不开工。开工前
  四步：① 知彼（了解项目是什么/什么功能/什么栈）② 调研（搜网上最新方案找
  最合适的现成工具，不自研轮子）③ 定策（**先定测试类型黑盒/白盒/自动化，
  再按项目类型选工具**：黑盒→postmcp/playwright/agent-tty/数据库 MCP/模糊，
  白盒→语言专项 JUnit/JaCoCo/SpotBugs 等，自动化→项目测试命令+自研工具；
  **只装当前项目需要的**，复杂项目多工具按模块分配协作）④ 备粮（只装需要
  且缺的工具）。工具没就绪不开始挖掘。
